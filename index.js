 ...
 * Mobile (iOS): single tap shows buttons for ~4s (auto-hide), double tap opens lightbox.
 */
function wrapImageWithActions(imgElement) {
    ...
    if (IS_IOS) {
        ...
        wrapper.addEventListener('click', (e) => {
            ...
            if (now - _lastTapTime < DOUBLE_TAP_THRESHOLD) {
                // Double tap → lightbox
            ...
            _tapTimer = setTimeout(() => {
                // Single tap → toggle action buttons
                e.preventDefault();
                e.stopPropagation();
...
/**
 * SVG icon for retry button on error placeholders.
 */
const SVG_ICON_RETRY = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>`;

/**
 * Create error placeholder element — shows error.svg with a retry button overlay.
 */
function createErrorPlaceholder(tagId, errorMessage, tagInfo) {
    ...
    // Extract instruction for retry capability
    ...
    if (tagInfo.fullMatch) {
        ...
        if (instructionMatch) {
            instructionValue = instructionMatch[2] || instructionMatch[1];
            img.setAttribute('data-iig-instruction', instructionValue);
        }
    }

    // If we have instruction data, wrap in a container with a retry button
    if (instructionValue) {
        const wrapper = document.createElement('div');
        wrapper.className = 'iig-error-wrapper';

        const retryBtn = document.createElement('button');
        retryBtn.className = 'iig-error-retry-btn';
        retryBtn.innerHTML = `${SVG_ICON_RETRY}<span>Retry</span>`;
...
function wrapExistingImages() {
    const images = document.querySelectorAll('#chat .iig-generated-image');
    let count = 0;
...
function initImageWrapObserver() {
    ...
    const processPending = () => {
        ...
        for (const node of nodes) {
            ...
            const imgs = node.classList?.contains('iig-generated-image')
                ? [node]
                : node.querySelectorAll?.('.iig-generated-image') || [];
...
function initLightbox() {
    ...
    // Desktop: single click on image opens lightbox (buttons handle their own clicks)
    // Mobile (iOS): handled by wrapImageWithActions (double tap = lightbox)
    document.getElementById('chat')?.addEventListener('click', (e) => {
        ...
        // On iOS, the wrapper handles tap/double-tap — don't interfere
        if (IS_IOS && img.closest('.iig-image-wrapper')) return;
